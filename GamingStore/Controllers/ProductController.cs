...
namespace GamingStore.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Browse(string name, string sortOrder)
        {
            try
            {
                var categoryGroups = new Dictionary<string, List<ProductCategory>>
                {
                    ...
                    ["Accessories"] = new() { ProductCategory.Keyboards, ProductCategory.Mice, ProductCategory.Headsets,
                             ProductCategory.Controllers, ProductCategory.GamingAccessories },
                    ["Others"] = new() { ProductCategory.GamingChairs, ProductCategory.GamingDesks, ProductCategory.Monitors,
                        ProductCategory.VRHeadsets, ProductCategory.PCComponents, ProductCategory.GamingMerchandise }
                };

                IQueryable<Product> productsQuery = _db.Products.Include(p => p.Images);
                ...
                {
                    productsQuery = productsQuery.Where(p => p.Category == singleCategory);
                    ViewBag.CategoryName = singleCategory.GetDisplayName() ?? name;
                }
                else
                {
                    return NotFound();
            ...
            {
                _logger.LogError(ex, "Error browsing products");
                TempData["ErrorMessage"] = "An error occurred while browsing products";
